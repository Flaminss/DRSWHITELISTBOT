...
module.exports = {
	...
	async userownsasset(userid, assetid, interaction) {
		const options = {
			url: 'https://inventory.roblox.com/v1/users/' + String(userid) + '/items/Asset/' + String(assetid) + '/is-owned',
			method: 'GET',
			headers: {
				'Accept': 'text/html',
				'User-Agent': 'Chrome',
		...
		return request(options).then((response) => {
			const parsedresponse = JSON.parse(response);
			if (parsedresponse.errors) return { success: false, error: 'Roblox API error: ' + parsedresponse.errors[0].message };
	...
	async getrobloxidusername(usern, interaction) {
		if (typeof usern == 'string') {
			...
			return request(options).then((response) => {
				const parsedresponse = JSON.parse(response);
				if (parsedresponse.error) return { success: false, error: 'Roblox API error: ' + parsedresponse.error };
				return { success: true, user: parsedresponse.data[0].id, name: usern };
			})
				.catch(function (err) {
	...
	async getrobloxid(usern, interaction) {
		...
		if (usern.startsWith('<@') && usern.endsWith('>')) {
			const member = getUserFromMention(usern, interaction);
			usern = member.nickname || member.user.username
			const options = {
				url: 'https://users.roblox.com/v1/usernames/users',
				method: 'POST',
				body: JSON.stringify({
			...
			if (typeof usern == 'number' && Number(usern) != null) {
				const options = {
					url: 'https://users.roblox.com/v1/usernames/users',
					method: 'POST',
					body: JSON.stringify({
			...
			return request(options).then((response) => {
				const parsedresponse = JSON.parse(response);
				if (parsedresponse.error) return { success: false, error: 'Roblox API error: ' + parsedresponse.error };
				return { success: true, user: parsedresponse.data[0].id, name: sending };
			})
				.catch(function (err) {
					return { success: false, error: 'Error:' + err };
				});
	...
	async getgroupinfo(groupid, interaction) {
		const options = {
		...
		return request(options).then((response) => {
			const parsedresponse = JSON.parse(response);
			if (parsedresponse.errors) return { success: false, error: 'Roblox API error: ' + parsedresponse.errors[0].message };
			return { success: true, name: parsedresponse.name, };
		})
			.catch(function (err) {
				return { success: false, error: 'Roblox API error:' + err };
			});
